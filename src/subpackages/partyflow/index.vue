...
const EVENT_CODE_LABEL: Record<string, string> = {
  status_initialized: '初始化',
  status_patched: '更新阶段',
  status_import: '批量导入',
  party_publicity_done: '公示完成',
  manual_test: '手动事件',
  league_applicant_talk_30d: '入团谈话提醒',
  league_activist_train_90d: '积极分子培养提醒',
  league_development_publicity_5workdays: '发展对象公示提醒',
  league_approval_30d: '审批提醒',
  league_member_archive_30d: '归档提醒',
  party_applicant_talk_30d: '入党谈话提醒',
  party_activist_report_every_90d: '思想汇报提醒',
  party_development_training_reminder: '培训提醒',
  party_probationary_transfer_365d: '转正提醒',
  party_probationary_report_every_90d: '预备期汇报提醒',
}
...
const reminderEvents = computed(() => {
  ...
  return out.sort((a, b) => compareIsoDesc(a.happened_at, b.happened_at))
})
...
function codeLabel(code?: string) {
  ...
  return EVENT_CODE_LABEL[raw] || raw
}
...
function stagesFor(item: PartyflowMyStatus) {
  ...
  const resolved = currentIndex >= 0 ? order : [...order, item.status].filter(Boolean)
  const resolvedIndex = resolved.indexOf(item.status)
  return resolved.map((status, index) => ({
    status,
    name: stageLabel(status),
    done: resolvedIndex >= 0 && index <= resolvedIndex,
    current: resolvedIndex === index,
  }))
}

function progressText(item: PartyflowMyStatus) {
  const order = STAGE_ORDER[item.org_type] || []
...
function sortedHistory(item: PartyflowMyStatus) {
  const list = Array.isArray(item.history) ? item.history : []
  return [...list].sort((a, b) => compareIsoDesc(a.happened_at, b.happened_at))
}

function visibleHistory(item: PartyflowMyStatus) {
  const list = sortedHistory(item)
...
function toggleHistory(item: PartyflowMyStatus) {
  expandedHistory.value[item.id] = !expandedHistory.value[item.id]
}
...
function eventTitle(evt: PartyflowEvent) {
  ...
  const code = codeLabel(evt.event_code)
  if (code) {
    return code
  }
...
function eventDesc(evt: PartyflowEvent) {
  return formatDateTime(evt.happened_at)
}
  ...
  <layout-shell current="partyflow">
    <view class="page-container partyflow-page">
      ...
      <content-panel v-for="item in statuses" :key="item.id" :title="`${orgLabel(item.org_type)}流程`" :sub-title="`当前阶段：${stageLabel(item.status)}`">
        <template #default>
          <text class="meta">阶段开始：{{ formatDateTime(item.status_started_at) }}</text>
          <text class="meta">预计动作：{{ item.next_action_hint || '暂无' }}</text>
          <text class="meta">{{ progressText(item) }}</text>
          ...
          <view class="history-wrap">
            <view class="history-head">
              ...
              <nut-button
                v-if="(item.history || []).length > 3"
                size="small"
                plain
                @click="toggleHistory(item)"
              >
                {{ expandedHistory[item.id] ? '收起' : '展开' }}
              </nut-button>
            </view>
            <nut-cell
              v-for="evt in visibleHistory(item)"
              :key="`${item.id}-${evt.id}`"
      ...
      <content-panel title="关键提醒">
        <template #default>
          <nut-cell
            ...
            :key="`reminder-${evt.id}`"
            :title="eventTitle(evt)"
            :desc="`${orgLabel(evt.org_type)} · ${eventDesc(evt)}`"
            icon="notice"
          />
